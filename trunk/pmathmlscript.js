...
mrowStretch (opid, opt, ope, opm, opb)
{
  ...
  if (mrowH > opH * 2)
      ...
      for (var i = 3; i * 2 * opH <= mrowH; i++)
	{
      ...
      opObj.html ('<span class="lr">' + symbol (opt) + '<br />' + es + m +
		  es + symbol (opb) + '</span>');
    }
}
/*
...
mfrac (aid, bid, type)
{
  var aObj = $ ('#' + aid);
  var bObj = $ ('#' + bid);
var aBox = { width:aObj.outerWidth (true) };
var bBox = { width:bObj.outerWidth (true) };
  if (type == 'full')
    {
  ...
  if (aBox.width > bBox.width)
    {
      aObj.css (
	      {
    borderStyle: 'solid', borderColor: '#000', borderWidth:'0 0 1px'}
    )}
  else
    {
      bObj.css (
	      {
    borderStyle: 'solid', borderColor: '#000', borderWidth:'1px 0 0'}
      );
    }
}
...
*/
var Queue = function (){ this.queue = new Array ();
  this.length = 0;
  this.idx = 0;
  var self = this;
